...
<a href="https://www.langchain.com/" target="_blank"><img src="https://img.shields.io/badge/LangChain-1.4+-1C3C3C?logo=langchain&logoColor=1C3C3C" alt="LangChain"></a>
...
<a href="https://www.langchain.com/" target="_blank"><img src="https://img.shields.io/badge/LangChain-1.4.4-1C3C3C?logo=langchain&logoColor=1C3C3C" alt="LangChain"></a>
...
- 专为亲子教育场景优化的系统提示词
- 实时流式响应，打字机效果展示
- 基于 LangChain 的对话链，支持会话管理和历史记录

#### 知识问答（RAG）
- 基于 LangChain 和向量数据库的知识库问答
- 支持多学科知识库（数学、语文、英语、科学）
- 使用 HNSWLib 向量存储实现语义检索
- 会话记忆功能，支持上下文理解
- 自动清理过期会话，优化资源占用

#### AI 作业辅导 Agent
- 基于 LangChain ReAct Agent 的智能作业辅导
- 集成数学计算、语文解释、英语翻译等工具
- 支持学科选择（数学、语文、英语、科学）
- 支持学生水平分级（小学、初中、高中）
- 会话隔离，不同学科独立记忆
...
| 🎨 主题切换 | 多种配色主题，个性化体验 |
| 🔒 安全认证 | SVG 验证码、密码加密、Token 过期自动跳转 |
...
│   │   │   ├── HomeworkAgent.jsx       # AI 作业辅导 Agent 页
│   │   │   ├── KnowledgeQA.jsx         # 知识问答（RAG）页
│   │   │   ├── Feedback.jsx            # 意见反馈页
│   │   │   ├── StudyProgress.jsx       # 学习进度页
│   │   │   ├── MinePage.jsx            # 个人中心
...
│   │   │   ├── chatHistory.js      # 对话历史本地存储
│   │   │   ├── tts.js              # TTS 语音朗读工具
│   │   │   └── loadingManager.js   # 全局加载状态管理
│   │   ├── Hooks/                  # 自定义 Hooks
...
│   │   │   ├── langchainController.js      # LangChain 对话控制器
│   │   │   ├── streamChatController.js     # SSE 流式对话控制器
│   │   │   ├── knowledgeRAGController.js   # 知识问答 RAG 控制器
│   │   │   ├── homeworkAgentController.js  # AI 作业辅导 Agent 控制器
│   │   │   ├── learningProgressController.js # 学习进度控制器
│   │   │   └── feedbackController.js       # 意见反馈控制器
│   │   ├── Models/              # 数据模型层
│   │   │   └── userModel.js     # 用户模型
│   │   ├── Routes/              # 路由层
│   │   │   ├── authRoutes.js         # 认证路由
...
│   │   │   ├── langchainAPI.js       # LangChain 对话路由
│   │   │   ├── homeworkAgentAPI.js   # 作业辅导 Agent 路由
│   │   │   ├── knowledgeRAGAPI.js    # 知识问答 RAG 路由
│   │   │   ├── feedbackAPI.js        # 意见反馈路由
│   │   │   └── learningProgressAPI.js # 学习进度路由
│   │   ├── Utils/               # 工具函数
│   │   │   ├── captcha.js       # 验证码工具
│   │   │   ├── jwt.js           # JWT 工具
│   │   │   ├── langchainConfig.js  # LangChain 配置与会话管理
│   │   │   └── sse.js           # SSE 流式响应工具
│   │   └── index.js             # 服务入口
│   └── package.json
│
...
### 2. LangChain 对话链与流式响应

基于 LangChain 构建带记忆功能的 AI 对话系统：

**后端 - LangChain 配置与会话管理**

```javascript
// Utils/langchainConfig.js
...
const createChatChain = (sessionId, options = {}) => {
    const llm = createLLM(options.modelName);
    ...
    return new RunnableWithMessageHistory({
        runnable: chain,
        getMessageHistory: () => chatSessionManager.getSessionHistory(sessionId),
